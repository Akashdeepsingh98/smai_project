...
class nueral_custom:
    ...
    def model(self,X, Y, n_h, num_iters, alpha, print_cost):
        np.random.seed(3)
        n_x = layers(X, Y)[0]
        n_y = layers(X, Y)[1]
        ...
        for i in range(0, num_iters):
            ...
            grads = self.back_prop(parameters, cache, X, Y)
            if (i > 20000):
                alpha1 = (20000 / i) * alpha
                parameters =self. update_params(parameters, grads, alpha1)
            else:
                parameters = self.update_params(parameters, grads, alpha)
        ...
        predictions = self.predict(parameters, X_test)
...
xcla=nueral_custom()
d = xcla.model(train_x.T, train_y.T, n_h=20, num_iters=50000, alpha=0.0002, print_cost=True)
